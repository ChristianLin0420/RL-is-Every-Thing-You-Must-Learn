...
the action-value function Q(s,a) for the current policy being followed.
...
class SarsaAgent:
    """
    SARSA (On-Policy) Agent for tabular environments.
    
    SARSA learns the Q-function for the behavior policy it follows,
    making it an on-policy method. It uses the actual next action a'
    selected by the policy for updates.
    """
    ...
    def update_q_value(
        ...
        # Next Q-value (using the actual next action selected by policy)
        if done:
    ...
    def train_episode(self, env: gym.Env, max_steps: int = 1000) -> Dict[str, Any]:
        """
        Train agent for one episode using SARSA algorithm.
        ...
        """
        state, _ = env.reset()
        action = self.epsilon_greedy_policy(state)  # Select first action
    ...
    def train(
        ...
        """
        if verbose:
            print(f"🎓 Training SARSA Agent for {n_episodes} episodes...")
